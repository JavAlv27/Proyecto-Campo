...
function renderExpensesList() {
    ...
    list.forEach(item => {
        ...
        li.innerHTML = `<div class="expense-info"><strong>${item.name}</strong><span>📅 ${item.date}</span></div><div class="expense-amount">-$${parseInt(item.amount).toLocaleString()}</div>`;
...
expenseForm.addEventListener('submit', (e) => {
    ...
    const data = { name: expenseNameInput.value, amount: expenseAmountInput.value, date: id ? expenses[id].date : todayStr, timestamp: id ? expenses[id].timestamp : timestamp };
...
function renderCalendar(animationClass = null) {
    calendar.innerHTML = "";
    monthYear.innerText = new Date(currentDate).toLocaleString('es-ES', { month: 'long', year: 'numeric' });
    calendar.className = 'calendar-grid'; 
    if(animationClass) { void calendar.offsetWidth; calendar.classList.add(animationClass); }
...
document.getElementById('nextMonth').onclick = () => { currentDate.setMonth(currentDate.getMonth()+1); renderCalendar('slide-next'); };
yearFilter.addEventListener('change', (e) => { selectedDashboardYear = parseInt(e.target.value); updateDashboardKPIText(); updateDashboard(true); });
